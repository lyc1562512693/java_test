...
public class ClassicOrder {
    ...
    public static int[] arr = new int[]{7,6,4,5,3,9,2,8};
    ...
    public static void insertSort(){
        for(int i = 1;i<arr.length;i++){
            int num = arr[i];
            int j = i;
            for(; j >= 1 && num<arr[j-1]; j--){//把添加放在for()里面判断和在for{}里面判断的区别是，在()里不满足直接结束，不会执行j--，而在for{}里面的if判断不满足后，则还会继续执行一次j--
                arr[j] = arr[j-1];
            }
            arr[j] = num;
        }
    ...
    public static void shellSort() {
        ...
        while (gap > 0) {
            for (int i = gap; i < arr.length; i++) {
                int num = arr[i];
                int j = i;
                for (; j >= gap && num > arr[j - gap]; j -= gap) {// 把添加放在for()里面判断和在for{}里面判断的区别是，在()里不满足直接结束，不会执行j--，而在for{}里面的if判断不满足后，则还会继续执行一次j--
                    arr[j] = arr[j - gap];
                }
                arr[j] = num;
            }
